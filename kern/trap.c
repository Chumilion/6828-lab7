...
#define SG_F_IRQ(num) SETGATE(idt[IRQ_OFFSET + num], 0, GD_KT, F_IRQ##num, 0);
#define DEC_F_IRQ(num) extern void F_IRQ##num ();
...
trap_init_percpu(void)
{
	...
	struct Taskstate *thists;
	int i;
	thists = &(thiscpu->cpu_ts);
	i = thiscpu->cpu_id;
...
page_fault_handler(struct Trapframe *tf)
{
	...
	uint32_t stacktop = ((UXSTACKTOP - PGSIZE < tf->tf_esp) &&
	...
	struct UTrapframe *utframe = (struct UTrapframe *) stacktop;
	utframe->utf_esp = 5;
